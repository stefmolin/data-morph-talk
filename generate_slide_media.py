...
def generate_bald_spot_example():
    ...
    fig.tight_layout()
    fig.savefig(MEDIA_DIR / "bald_spots.png", facecolor="white", bbox_inches="tight")
    plt.close(fig)
...
def generate_simulated_annealing_animation():
    ...
    def reframe(axs):
        for ax in axs:
            ax.set(xlim=dataset.plot_bounds.x_bounds, ylim=dataset.plot_bounds.y_bounds)
    ...
    def update(frame):
        ...
        for ax, alpha in zip(axs, alphas):
            ...
            if frame % 2 == 0:  # show where change will happen
                ...
                ax.scatter(current.loc[idx].x, current.loc[idx].y, s=20, color="red")
